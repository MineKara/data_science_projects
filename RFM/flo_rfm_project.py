...
rfm.loc[rfm["RFM_score"] == "55"].head(10)
...
rfm_copy = rfm.reset_index()


rfm_df = pd.merge(rfm, df[["interested_in_categories_12","order_channel" ,"master_id"]], on=["master_id"])


## Write a csv file to a new folder

from pathlib import Path
filepath = Path('D:/13th Data Science Bootcamp/3Week_CRM_Analytics/rfm_copy.csv')
filepath.parent.mkdir(parents=True, exist_ok=True)
rfm_copy.to_csv(filepath)
...
################################################################################################################
# Alternative
potential_customer_id = rfm.loc[(rfm["segment"]=="champions") | (rfm["segment"]=="loyal_customers")].reset_index()["master_id"]
target_customer = df[df["master_id"].isin(potential_customer_id)].loc[(df["interested_in_categories_12"]).str.contains("KADIN")]["master_id"]
target_customer.to_csv("target_customer.csv", index=False)
...
high_potential_customer_id = rfm[(rfm["segment"]=="cant_loose") | (rfm["segment"]=="about_to_sleep") | (rfm["segment"]=="new_customers")].reset_index()["master_id"]
target_men_customer = df[df["master_id"].isin(high_potential_customer_id)].loc[(df["interested_in_categories_12"]).str.contains("ERKEK") | (df["interested_in_categories_12"]).str.contains("COCUK")]["master_id"]
target_men_customer.to_csv("target_men_customer.csv",index=False)
...
seg_map = {
    ...
    r'[1-3][3-5][3-5]':'cant_loose',
    r'[2-3][1-3][1-3]': 'awake_segment',
    r'[2-4][3-4][2-4]': 'potential_segment',
    r'[3-5][4-5][4-5]': 'top_segment'
}
...
rfm_top_segment = rfm1[rfm1["segment"].isin(["top_segment","potential_segment","cant_loose","new_customer","awake_segment"])]
rfm_top_segment[["segment", "Recency", "Frequency", "Monetary"]].groupby("segment").agg(["mean", "count"])